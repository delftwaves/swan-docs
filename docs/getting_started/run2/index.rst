...
The user can either choose between running SWAN directly using docker or the docker in interactive mode (flag ``-it``).
...
.. note::
   ...
   - The flag ``--rm`` removes the exited container from your machine after terminating SWAN. (Check by invoking the command ``docker ps -a``.)
